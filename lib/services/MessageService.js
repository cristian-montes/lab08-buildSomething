...
class MessageService {
  ...
  //UPDATES QUOTES BY IDE FROM DATA BASE
  static async updateQuoteById(id, quote){
    const quuote = await Message.updateById(id, quote);
    return quuote;
  }

  //DELETES QUOTE BY ID FROM DATA BASE
  static async deleteQuoteById(id){
    const quuote = await Message.deleteById(id);
    return quuote;
  }
